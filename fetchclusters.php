...

{
  ...
  $movie_id[]=$row[0];
	...
	$avguserret="";
	$urlret="";
$query="SELECT cluster from clusters_5 where usn<='$usn'+0.5 and usn>='$usn'-0.5 ";
$result = mysqli_query($db_server,$query);
if (!$result) die ("Database access failed: " . mysqli_error($db_server));
$rows = mysqli_num_rows($result);
$cluster=array();
...
$query2="SELECT c.usn from clusters_5 c where c.cluster ='$maxc'";
...
//echo $movies[0];
$query1 = "select distinct e.elective_name from subjects s,electives e where s.usn in ('" .implode("', '", $movies). "') and s.sem=$sem and s.subject4=e.elective_code LIMIT 20";
$query2 = "select distinct e.elective_name from subjects s,electives e where s.usn in ('" .implode("', '", $movies). "') and s.sem=$sem and s.subject5=e.elective_code LIMIT 20";
...

{
  ...
  //echo ;
  //$imagearray[]=$row[0];
  $GLOBALS['nameret']=$GLOBALS['nameret'].$row[0].",";
...

{
  ...
  $GLOBALS['criticret']=$GLOBALS['criticret'].$row[0].",";
...
$GLOBALS['ret']=$GLOBALS['ret'].$GLOBALS['nameret']."separator".$GLOBALS['criticret'];//."moviemaniasetseparator";
echo $GLOBALS['ret'];
